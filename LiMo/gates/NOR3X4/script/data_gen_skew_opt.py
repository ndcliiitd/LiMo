...
(procedure (simulation pcorners pvdd ptemp pload pslew_a pslew_b pslew_c  pskew_b pskew_c)
    ...
    if( (pskew_b >= 0 && pskew_c >= 0)  
        (if (pskew_b <= pskew_c)
             (tplh = delay( v("/C") pvdd/2.0 1 'falling v("/Y")  pvdd/2.0 1 'rising )) 
...
(procedure (min_length tphl_list previous_tphl_list)
    tphl_list_length = length(tphl_list)
    previous_tphl_list_length = length(previous_tphl_list)
    min_length = min(tphl_list_length previous_tphl_list_length)
)

;; Define a helper procedure to check the absolute difference between two values
(procedure (check_difference value1 value2 threshold)
    ((absolute (value1-value2)) <= threshold)
)


;; Define a named procedure to check elements recursively
(define (check_elements i min_length tphl_list previous_tphl_list threshold)
    (if (i >= min_length)
        t ; All elements are checked
        (progn
            (printf " tphl_list %L" tphl_list)  
            (setq tphl_list_element (conv_nil (nth i tphl_list)))
            (printf " tphl_list_element %L" tphl_list_element)  

            (printf " previous_tphl_list %L" previous_tphl_list)  
            (setq previous_tphl_list_element (conv_nil (nth 0 (nth i previous_tphl_list))))
            ;;(setq previous_tphl_list_element (conv_nil (nth i previous_tphl_list)))
            (printf " previous_tphl_list_element %L" previous_tphl_list_element)

            (if (check_difference tphl_list_element previous_tphl_list_element threshold)
                (check_elements (1+ i) min_length tphl_list previous_tphl_list threshold)
                nil ; Element difference greater than threshold
            )
        )
    )
)
...
(defvar tphl 1e-15)
(defvar tphl_list '())
(defvar previous_tphl_list '())
(defvar i 0)

;; Initialize breakSimulationCount variables
(setq breakSimulationCount 0)
(setq breakSimulationCount_1 0)


;; Define input parameter lists
...
foreach((pskew_c) pskew_c_1
    (if (pskew_c < 0)
        (pskew_c_before_zero = append(pskew_c_before_zero list(pskew_c)))
        (pskew_c_after_zero = append(pskew_c_after_zero list(pskew_c))))
    )
)

pskew_c_before_zero = reverse(pskew_c_before_zero)
 		       ...
 		       (setq breakSimulationCount_1 0)
   			   ...
   			   (setq breakSimulationCount_1 0)
                           ...
                           foreach( (pskew_b) pskew_b_before_zero
                               ...
                               (setq breakSimulationCount 0)
			       when( (breakSimulationCount_1 <= 3)

				   ;; Reset tphl_list and previous_tphl_list after the outer loop for pskew_b
				   (setq previous_tphl_list '())                                   

				   (setq previous_tphl_list (append previous_tphl_list (list tphl_list)))
			            printf(" previous_list %L", previous_tphl_list)     
				   
                                   ;; Reset tphl_list and previous_tphl_list after the outer loop for pskew_b
				   (setq tphl_list '())

                                   foreach( (pskew_c) pskew_c_before_zero
					   ...
					   ;; Convert nil values to numbers if needed
					   ;;if anywhere in dataset generataing the nil convert it into number so that subtarction operation can be performed latter on.
				           ...
				           (setq tphl_list (append tphl_list (list tphl)))
                                            printf("current list ", tphl_list)
                                            printf(" previous_list", previous_tphl_list)  
					   ...
					   (fprintf of "%e,%e,%e,%e,%s,%f,%L,%L,%L,%L,%L,%L,%L\n", pload pslew_a pslew_b pslew_c pcorners pvdd ptemp pskew_b pskew_c tplh tphl tr tf)
     				           ;; Check for consecutive same delay values for skew_c
					   ...
					   ;; Update previous values
				           (setq previous_tphl tphl)
					   (setq previous_tphl (conv_nil previous_tphl))
					
                                       ;;end skew_c when loop
			 	       )    								             
                                   ;;end skew_c loop
                      		   )

				   ;; Check for consecutive same delay for skew_b
				   (min_length tphl_list previous_tphl_list)
				   if( (check_elements i min_length tphl_list previous_tphl_list threshold)
			               (setq breakSimulationCount_1 (plus breakSimulationCount_1 1))
				       (setq breakSimulationCount_1 (plus breakSimulationCount_1 0))
                                   )
                             
                               ;;end skew_b when loop
                               )
			    ;;end skew_b loop
                            )
   			   ...
   			   (setq breakSimulationCount_1 0)
				   ...
				   (setq tphl_list '())
                                   ...
                                   foreach( (pskew_c) pskew_c_after_zero
                                       desVar("skew_c", pskew_c)
		                       when( (breakSimulationCount <= 3)
...
interim_output_file = "interim/interim_dataset_skew.csv"
...
def simulate(ocean_script, output_file, simulator_name, design_dir, results_dir, model_file, stimulus_file, analysis_start, analysis_stop, analysis_step, load_1, slew_a_1, slew_b_1, slew_c_1):
    ...
    print("Standard Output NOR3X4:")
    print( ocean_script_output.stdout)

    print("Standard Error NOR3X4:")
    ...
    if ocean_script_output.returncode == 0:
        print("Ocean environment started successfully NOR3X4.")
    else:
        print("Error: Failed to start ocean environment NOR3X4")
    ...
    for slew_a_1 in pslew_a:
        ...
        for slew_b_1 in pslew_b:
            ...
            for slew_c_1 in pslew_c:
                ...
                index = index + 1
                print("DATASET GENERATION STARTED FOR GIVEN INPUT NOR3X4")
...
print("\n**************************************\n")
print("\nDATASET GENERATION COMPLETED NOR3X4\n")

# Stop the overall timer
total_end_time = time.time() 
total_elapsed_time = total_end_time - total_start_time
print(f"Total simulation time NOR3X4: {total_elapsed_time} seconds")
