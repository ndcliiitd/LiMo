...
(procedure (min_length tplh_list previous_tplh_list)
    tplh_list_length = length(tplh_list)
    previous_tplh_list_length = length(previous_tplh_list)
    min_length = min(tplh_list_length previous_tplh_list_length)
)
...
(define (check_elements i min_length tplh_list previous_tplh_list threshold)
    (if (i >= min_length)
        t ; All elements are checked
        (progn
            (printf " tplh_list %L" tplh_list)  
            (setq tplh_list_element (conv_nil (nth i tplh_list)))
            (printf " tplh_list_element %L" tplh_list_element)  

            (printf " previous_tplh_list %L" previous_tplh_list)  
            (setq previous_tplh_list_element (conv_nil (nth 0 (nth i previous_tplh_list))))
            ;;(setq previous_tplh_list_element (conv_nil (nth i previous_tplh_list)))
            (printf " previous_tplh_list_element %L" previous_tplh_list_element)

            (if (check_difference tplh_list_element previous_tplh_list_element threshold)
                (check_elements (1+ i) min_length tplh_list previous_tplh_list threshold)
 		       ...
 		       (setq breakSimulationCount_1 0)
   			   ...
   			   (setq breakSimulationCount_1 0)
                           ...
                           foreach( (pskew_b) pskew_b_before_zero
                               ...
                               (setq breakSimulationCount 0)
			       when( (breakSimulationCount_1 <= 2)

				   ;; Reset tplh_list and previous_tplh_list after the outer loop for pskew_b
				   (setq previous_tplh_list '())                                   

				   (setq previous_tplh_list (append previous_tplh_list (list tplh_list)))
			            printf(" previous_list %L", previous_tplh_list)     
				   ...
				   (setq tplh_list '())
					   ...
					   ;;if anywhere in dataset generataing the nil convert it into number so that subtarction operation can be performed latter on.
				           ...
				           (setq tplh_list (append tplh_list (list tplh)))
                                            printf("current list ", tplh_list)
                                            printf(" previous_list", previous_tplh_list)  
					   ...
					   ;; Update previous values
				           (setq previous_tplh tplh)
					   (setq previous_tplh (conv_nil previous_tplh))
				   ...
				   (min_length tplh_list previous_tplh_list)
				   if( (check_elements i min_length tplh_list previous_tplh_list threshold)
...
def simulate(ocean_script, output_file, simulator_name, design_dir, results_dir, model_file, stimulus_file, analysis_start, analysis_stop, analysis_step, load_1, slew_a_1, slew_b_1, slew_c_1):
    ...
    print("Standard Output NAND3XL:")
    print( ocean_script_output.stdout)

    print("Standard Error NAND3XL:")
    ...
    if ocean_script_output.returncode == 0:
        print("Ocean environment started successfully NAND3XL.")
    else:
        print("Error: Failed to start ocean environment NAND3XL")
    ...
    for slew_a_1 in pslew_a:
        ...
        for slew_b_1 in pslew_b:
            ...
            for slew_c_1 in pslew_c:
                ...
                index = index + 1
                print("DATASET GENERATION STARTED FOR GIVEN INPUT NAND3XL")
...
print("\n**************************************\n")
print("\nDATASET GENERATION COMPLETED NAND3XL\n")

# Stop the overall timer
total_end_time = time.time() 
total_elapsed_time = total_end_time - total_start_time
print(f"Total simulation time NAND3XL: {total_elapsed_time} seconds")
