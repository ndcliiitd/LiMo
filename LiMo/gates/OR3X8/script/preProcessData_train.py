...
def load_data_X_train(data_path):
    ...
    data_head = ['load', 'slew_a', 'slew_b','slew_c', 'process', 'voltage', 'temperature', 'skew_b','skew_c']
...
# Convert the relevant columns to numerical type if they are not already
...
def load_data_y_train(data_path):
    ...
    # Define column headers
    data_head = ['rise_delay', 'fall_delay', 'rise_slew', 'fall_slew']
...
if __name__ == "__main__":
    ...
    #Preprocess the data
    preprocessed_X_train = preprocess_data(X_train, X_train)
    preprocessed_y_train = preprocess_data(y_train, y_train)

    data_path_preprocessed_X_train = os.path.join(output_directory, gate_name, "preprocessed_X_train.csv")
    preprocessed_X_train.to_csv(data_path_preprocessed_X_train, index = False)

    data_path_preprocessed_y_train = os.path.join(output_directory, gate_name, "preprocessed_y_train.csv")
    preprocessed_y_train.to_csv(data_path_preprocessed_y_train, index = False)
