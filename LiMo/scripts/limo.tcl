...
proc display_welcome_message {} {
    puts "-------------------------------------------------"
    puts "Welcome to LiMo!"
    puts ""
    puts "LiMo - An Intelligent Library Model"
    puts ""
    puts "LiMo is a tool for creating Intelligent Library Models for digital applications."
...
proc display_available_commands {} {
    ...
    puts "2. getLibCells                                                                 - Lists available combinational logic gates in the directory \$lm_gate_dir."
...
"
    puts "6. setInput -gate GATENAME                                                     - Opens user_input.py file for a specified gate using gedit for reading/editing."
    puts "7. genDataset -gate GATENAME OPTIMIZATION_METHOD -num_processes NUM_OF_CORES   - Initiates dataset generation for selected gates with options including multiprocessing and optimization methods (none, skew_opt, skew_slew_op   t). e.g usage : genDataset -gate NAND2X1 NOR2X1 -optimize none -num_processes 2."
    puts "8. viewOutput -gate GATENAME file_name FILENAME                                - Opens the FILENAME file for a specific gate using the default CSV viewer. "
    puts "9. loadData -gate GATENAME -file_name FILENAME                                 - Loads the FILENAME file for a specific gate using the default CSV viewer."
    puts "10. plotData -gate GATENAME -file_name FILENAME                                 - Generates plot to visualize the dataset."
    puts "11. infoData -gate GATENAME -file_name FILENAME                                 - Displays and saves data for the specified gate"
    puts "12. splitData -gate GATENAME -file_name FILENAME -test_size TESTSIZE            - Split the data of the FILENAME and saves it in a temporary directory for the specifed gate"
    puts "13. preProcessData_train -gate GATENAME                                         - Preprocesses data for training for the specifed gate."
    puts "14. preProcessData_test -gate GATENAME               		               - Preprocesses data for testing for the specifed gate."
    puts "16. trainModel -gate GATENAME -output MODELNAME     		               - Train a machine learning model for the specifed gate and save the trained model."
    puts "16. testModel -gate GATENAME -load MODELNAME -report REPORTNAME                - Tests a machine learning model for a gate and generates a report."
    puts "17. exit                                                                       - Exit the tool LiMo."
}
...
# Function to open the user_input.tcl file for a specific gate using the default text editor
...
proc gen_dataset {args} {
    ...
    foreach arg $args {
        ...
        switch -- $arg {
            ...
            "-optimize" {
                set optimization [lindex $args [expr [lsearch $args $arg] + 1]]
                set skip_next_arg 1
            }
            "-num_processes" {
    ...
    if {$gate_specified == 0 || [llength $gate_names] == 0} {
        puts "Usage: genDataset -gate GATENAME [GATENAME...]  -optimize OPTIMIZATION -num_processes NUM"
        return
    }
    ...
    if {[catch {exec $python_executable $python_script -gate [join $gate_names ,] -optimize $optimization -num_processes $num_processes} result]} {
...
#genDataset -gate NOR2X1 OR2X1 -optimize none -num_processes 2

# Function to open the dataset file for a specific gate
...
#viewDataset -gate NAND2X1 -file_name GPr3_main_dataset_NAND2X1.csv


# Function to load data for a specific gate
...
#plotData -gate NAND2X1 -file_name GPr3_main_dataset_NAND2X1.csv
# Function to info data for a specific gate
...
#infoData -gate NAND2X1 -file_name GPr3_main_dataset_NAND2X1.csv
...
#splitData -gate NAND2X1 -file_name GPr3_main_dataset_NAND2X1.csv -test_size 0.2

# Function to run preProcessData_train.py for a specific gate
...
#preProcessData_train -gate NAND2X1

# Function to run preProcessData_test.py for a specific gate
...
# Function to train the model for a specific gate and save it
...
#trainModel -gate NAND2X1 -output trained_model_NAND2X1

# Function to test the model for a specific gate and save the report
...
#testModel -gate NAND2X1 -load trained_model_NAND2X1.pkl -report report_NAND2X1.txt
...
# Execute the LiMo tool
proc execute_LiMo_tool {} {
    ...
    #set gates_directory "/home/poojabe/Desktop/PhD_Research/src_project/ml_lib_char/LiMo/gates"
    ...
    while {1} {
	    ...
	    "getVar" {
            ...
            "getLibCells" {
                # Call the list-gates function to list gates
            ...
            "setInput" {
                if {[llength $parts] == 3 && [lindex $parts 1] eq "-gate"} {
                    set gate_name [lindex $parts 2]
                    open_user_input $gate_name $gates_directory
                } else {
                    puts "Usage: setInput -gate GATENAME"
                }
            }
            "genDataset" {
            ...
            "viewDataset" {
    		if {[llength $parts] == 5 && [lindex $parts 1] eq "-gate" && [lindex $parts 3] eq "-file_name"} {
    		    set gate_name [lindex $parts 2]
    		    set file_name [lindex $parts 4]
    		    view_dataset $gate_name $file_name $output_directory
    		} else {
    		    puts "Usage: viewDataset -gate GATENAME -file_name FILE_NAME"
    		}
	    }
            "loadData" {
    		if {[llength $parts] == 5 && [lindex $parts 1] eq "-gate" && [lindex $parts 3] eq "-file_name"} {
    		    ...
    		    puts "Usage: loadData -gate GATENAME -file_name FILE_NAME"
    		}
	    }
	    "plotData" {
    		if {[llength $parts] == 5 && [lindex $parts 1] eq "-gate" && [lindex $parts 3] eq "-file_name"} {
    		    ...
    		    puts "Usage: plotData -gate GATENAME -file_name FILE_NAME"
    		}
	    }
            "infoData" {
    		if {[llength $parts] == 5 && [lindex $parts 1] eq "-gate" && [lindex $parts 3] eq "-file_name"} {
   		     ...
   		     puts "Usage: infoData -gate GATENAME -file_name FILE_NAME"
   		}
	    }
	    "splitData" {
    		if {[llength $parts] == 7 && [lindex $parts 1] eq "-gate" && [lindex $parts 3] eq "-file_name" && [lindex $parts 5] eq "-test_size"} {
    		    set gate_name [lindex $parts 2]
    		    set file_name [lindex $parts 4]
    		    set test_size [lindex $parts 6]
    		    split_data $gate_name $file_name $test_size $gates_directory $output_directory
    		} else {
    		    puts "Usage: splitData -gate GATENAME -file_name FILE_NAME -test_size TEST_SIZE"
    		}
	    }
	    "preProcessData_train" {
    		if {[llength $parts] == 3 && [lindex $parts 1] eq "-gate"} {
    		    ...
    		    puts "Usage: preProcessData_train -gate GATENAME"
    		}
	    }
	    "preProcessData_test" {
    		if {[llength $parts] == 3 && [lindex $parts 1] eq "-gate"} {
    		    ...
    		    puts "Usage: preProcessData_test -gate GATENAME"
    		}
	    }
	    "trainModel" {
                if {[llength $parts] == 5 && [lindex $parts 1] eq "-gate" && [lindex $parts 3] eq "-output"} {
                    set gate_name [lindex $parts 2]
                    set model_name [lindex $parts 4]
                    train_model $gate_name $model_name $gates_directory $output_directory
                } else {
                    puts "Usage: trainModel -gate GATENAME -output MODELNAME"
                }
            }
	    "testModel" {
    		if {[llength $parts] == 7 && [lindex $parts 1] eq "-gate" && [lindex $parts 3] eq "-load" && [lindex $parts 5] eq "-report"} {
    		    set gate_name [lindex $parts 2]
    		    set model_name [lindex $parts 4]
    		    set report_name [lindex $parts 6]
    		    test_model $gate_name $model_name $report_name $gates_directory $output_directory
    		} else {
    		    puts "Usage: testModel -gate GATENAME -load MODELNAME -report REPORTNAME"
    		}
	    }
            "help" {
